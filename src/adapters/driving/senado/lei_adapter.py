...
class LeiOrdinariaComplementarAdapter(FetcherPort):
    """
    Wrapper que adapta LeiAdapter para a interface FetcherPort.
    """
    
    def __init__(self, client: Optional[SenadoClient] = None):
        self.adapter = LeiAdapter(client)
        self.client = client or SenadoClient()
    
    def buscar(self, **kwargs: Any) -> FetchResult:
        """
        Busca uma Lei (Ordinária ou Complementar) específica.
        ...
        Returns:
            FetchResult com a lei encontrada
        """
        numero = kwargs.get("numero")
        ano = kwargs.get("ano")
        tipo = kwargs.get("tipo", "LEI")
        ...
        try:
            if tipo.upper() == "LC":
                norma = self.adapter.extrair_lei_complementar(numero, ano)
                titulo = f"Lei Complementar {numero}/{ano}"
            else:
                norma = self.adapter.extrair_lei_ordinaria(numero, ano)
                titulo = f"Lei {numero}/{ano}"
            ...
            if norma:
                ...
                return FetchResult(
                    success=False,
                    error=f"{tipo} {numero}/{ano} não encontrada"
                )
    ...
    def validar(self, identifier: str) -> bool:
        ...
        try:
            ...
            if len(parts) >= 7:
                ano = parts[5]
                numero = parts[6]
                # Tenta ambos os tipos
                norma = self.adapter.extrair_lei_ordinaria(int(numero), int(ano))
                if norma is None:
                    norma = self.adapter.extrair_lei_complementar(int(numero), int(ano))
    ...
    def fetch(self, params: Optional[Dict[str, Any]] = None) -> List[FetchResult]:
        """Executa fetch de Leis Ordinárias e Complementares."""
        results = []
        ...
        try:
            for year_offset in range(min(5, limit)):
                ...
                for num in range(1, min(3, limit // 5)):
                    # Tenta Lei Ordinária
                    try:
                        norma = self.adapter.extrair_lei_ordinaria(num, year)
                        if norma:
                            results.append(FetchResult(
                                success=True,
                                data={
                                    "urn": norma.urn,
                                    "title": norma.ementa or f"Lei {num}/{year}",
                                    "date": norma.data_promulgacao,
                                    "level": "L3",
                                    "metadata": norma.to_dict()
                                }
                    ...
                    # Tenta Lei Complementar
                    try:
                        norma = self.adapter.extrair_lei_complementar(num, year)
                        if norma:
                            results.append(FetchResult(
                                success=True,
                                data={
                                    "urn": norma.urn,
                                    "title": norma.ementa or f"Lei Complementar {num}/{year}",
                                    "date": norma.data_promulgacao,
                                    "level": "L3",
                                    "metadata": norma.to_dict()
                                }
